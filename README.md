...
The loader is a complement to the [svelte-loader](https://github.com/sveltejs/svelte-loader) to able to use CSS Modules classname on Svelte components. So make sure to install the `svelte-loader` along with the `svelte-cssmodules-loader`
...
The `svelte-cssmodules-loader` is doing all its work on a "raw" Svelte component (before the compilation). So make sure the loader get executed before `svelte-loader`.
...
**On the HTML markup** (not the CSS), Prefix any class name that require CSS Modules to by *$style.*  => `$style.My_CLASSNAME`
...
While the native CSS Scoped system should be largely enough to avoid class conflict, it could find its limit when working on a hybrid project. On a non full javascript front-end where Svelte is here to enhance pieces of the page, the thought on the naming of the component classes is no less different than the one on a regular html page. On the modal example above, It would have been wiser to namespace some of the classes such as `.modal-body` and `.modal-class` to avoid inheriting any styles from other `.body` and `.cancel`.
